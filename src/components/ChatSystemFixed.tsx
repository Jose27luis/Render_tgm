...
const ChatSystemFixed: React.FC = () => {
  ...
  return (
    <>
              ...
              <>
                <Box sx={{ 
                  ...
                  {messages.map((message) => (
                    <MessageBubble key={message.id_mensaje} message={message} />
                  ))}
      ...
      >
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 1, p: 1 }}>
          {EMOJIS.map((emoji) => (
            <MenuItem
              key={emoji}
...
export default ChatSystemFixed; 